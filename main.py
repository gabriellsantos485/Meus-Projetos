...
class Login:
    def __init__(self, root):
        ...
        #Background Image
        self.bg = ImageTk.PhotoImage(file="images/2.jpeg")
        ...
        # Title and subtitle
        title = Label(Frame_login, text="Login Here" , font=("Goudy old style", 35, "bold"), fg="#6162FF", bg="white").place(x=90, y=30)
        ...
        # Button
        forget = Button(Frame_login, text="forgot password ?", bd=0, font=('Goudy old style', 12),cursor="hand2", fg="grey",bg="white").place(x=90, y=280)
        submit = Button(Frame_login, command=self.check_function, text="Login?", cursor="hand2",bd=0, font=('Goudy old style', 15), bg="#6162FF", fg="white").place(x=90, y=320, width=180, height=40)

    def check_function(self):
        if self.username.get()=="" or self.password.get()=="":
            messagebox.showerror("Error", "All fields are required", parent=self.root)
        elif self.username.get()!="Gabriel" or self.password.get()!="123456":
